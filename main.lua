...
local MOD_ID = "expanded_exp_share_modes"
local PATCH_KEY = "__expanded_exp_share_modes_dispatch_v1"
...
return function(mod)
  ...
  local function distribute(original, battle)
    ...
    if mode ~= "off" and mode ~= "classic" and mode ~= "modern" and mode ~= "even" then
      mode = "classic"
    end
    ...
    -- Even Split: every living party member gets the full un-divided EXP share,
    -- as if they were the sole participant. Fighters get theirs via the normal
    -- participant flow; bench members get their full share added on top.
    if mode == "even" then
      ...
      -- Capture who the fighters are before we suppress EXP.
      local fighters = currentParticipants(battle)

      -- Suppress vanilla EXP_ALL so the participant flow below only awards
      -- to the actual fighters (not the whole bench).
      local result = pack(pcall(function()
        return callWithoutExpAll(battle, function() return original(battle) end)
      end))
      if not result[1] then error(result[2], 0) end

      -- Award full EXP to every living member who did NOT fight (bench only).
      -- Fighters already got their full share from the participant flow.
      local allLiving = livingParty(battle)
      for _, mon in ipairs(allLiving) do
        if fighters[mon] then
          -- Fighter: already handled by the participant flow.
        else
          -- Bench: give them the full undoubled share.
          local levels, gained = Experience.apply(
            battle.data, mon, defeatedDef, enemyLevel, isTrainer, 1, mon.traded)
